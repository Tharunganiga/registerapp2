...
stage("Trivy Scan") {
	...
	stage ('Cleanup Artifacts') {
	    ...
	    stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user Tharun ganiga:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'ec2-13-51-69-141.eu-north-1.compute.amazonaws.com:8080/job/gitops-register-app-cd/buildWithParameters?token=gitops-token'"
                }
            }
       }	
